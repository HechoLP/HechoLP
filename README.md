<div align="center">

# ⚡ dlfkd

**경북 포항에서 코드를 만드는 학생 개발자, 이랑입니다.**<br/>
처음 만든 버튼이 클릭됐을 때의 `"아 이게 되네."`라는 감각을 좋아합니다.

<br/>

<a href="https://github.com/HechoLP">
  <img src="https://img.shields.io/badge/GITHUB-@HechoLP-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
</a>
<a href="https://dlfkd.dev">
  <img src="https://img.shields.io/badge/WEBSITE-dlfkd.dev-0EA5E9?style=for-the-badge&logo=vercel&logoColor=white" alt="Website"/>
</a>

<br/><br/>

<img src="https://img.shields.io/badge/School-GBSW-64748B?style=flat-square" alt="GBSW"/>
<img src="https://img.shields.io/badge/Location-Pohang%2C_KR-0EA5E9?style=flat-square" alt="Pohang"/>
<img src="https://img.shields.io/badge/Projects-2-22C55E?style=flat-square" alt="Projects"/>
<img src="https://img.shields.io/badge/Open_to-Team_Project_%2F_Hackathon-F43F5E?style=flat-square" alt="Open to team project and hackathon"/>

</div>

---

## 👋 소개 (About Me)

- 안녕하세요, **경북소프트웨어마이스터고**에서 개발을 공부하고 있는 **이랑(Lee Rang)**입니다.
- 목표는 단순합니다. **내가 만든 걸 누군가 실제로 쓰는 것.**
- 웹과 백엔드를 중심으로 작은 아이디어를 서비스 형태로 만들고, 오류를 고치며 구조를 다듬는 과정을 좋아합니다.

---

## 🧩 주요 프로젝트 (Featured Projects)

### 매점 프로젝트

- 학교 매점의 주문과 재고 관리를 간편하게 만드는 키오스크 시스템입니다.
- **JA Korea 창업 프로젝트**의 지원을 받아 진행하고 있습니다.
- `역할: 백엔드 및 데이터베이스` · `React` · `Java` · `Supabase`
- 상태: `진행 중`

### CokZip

- 2026 GEEKS 해커톤에서 기획하고 개발에 참여한 학습 도움 튜터입니다.
- `역할: 기획 및 개발 참여` · `2026.08 GEEKS 해커톤 참가작`

---

## 🛠 기술 스택 (Tech Stack)

### Programming

`Python` · `C` · `Java`

### Web Development

`HTML` · `CSS` · `JavaScript` · `React`

### Tools / Data

`Git` · `GitHub` · `Linux` · `SQL` · `Neon` · `Supabase` · `Vercel`

<br/>

<div align="center">

<img src="https://img.shields.io/badge/-Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python"/>
<img src="https://img.shields.io/badge/-C-A8B9CC?style=for-the-badge&logo=c&logoColor=111111" alt="C"/>
<img src="https://img.shields.io/badge/-HTML-E34F26?style=for-the-badge&logo=html5&logoColor=white" alt="HTML"/>
<img src="https://img.shields.io/badge/-CSS-1572B6?style=for-the-badge&logo=css3&logoColor=white" alt="CSS"/>
<img src="https://img.shields.io/badge/-JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=111111" alt="JavaScript"/>
<img src="https://img.shields.io/badge/-React-61DAFB?style=for-the-badge&logo=react&logoColor=111111" alt="React"/>
<img src="https://img.shields.io/badge/-Java-007396?style=for-the-badge&logo=openjdk&logoColor=white" alt="Java"/>
<img src="https://img.shields.io/badge/-Linux-FCC624?style=for-the-badge&logo=linux&logoColor=111111" alt="Linux"/>
<img src="https://img.shields.io/badge/-Neon-000000?style=for-the-badge&logo=data:image/svg%2bxml;base64,PHN2ZyBmaWxsPSIjMDBFNTk5IiByb2xlPSJpbWciIHZpZXdCb3g9IjAgMCAyNCAyNCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48dGl0bGU%2bTmVvbjwvdGl0bGU%2bPHBhdGggZD0iTTI0IDBWMjRsLTkuMzY1LTguMDQ1VjI0SDBWMFpNMi45NDIgMjEuMDg3aDguNzUxVjkuNTYzbDkuMzY1IDguMjA0VjIuOTE5TDIuOTQyIDIuOTE0WiIvPjwvc3ZnPg%3d%3d" alt="Neon"/>
<img src="https://img.shields.io/badge/-Supabase-3FCF8E?style=for-the-badge&logo=supabase&logoColor=111111" alt="Supabase"/>

</div>

---

## 🤖 Prompt Engineering

- AI를 단순한 답변 도구가 아니라, **문제 정의와 해결 흐름을 정리하는 개발 도구**로 활용합니다.
- 요구사항을 구체화하고 코드 작성, 디버깅, 기획 과정에서 더 나은 결과를 얻는 프롬프트를 실험합니다.

---

## 🏅 자격증 (Certifications)

- **ITQ** | KPC 자격, 2025.01
- 아래한글 `A` · 한글엑셀 `B` · 한글파워포인트 `A`

---

## 🏆 활동 및 수상 (Activities & Awards)

### 경북소프트웨어마이스터고 1학년

- **2026.05 AI EXPO KOREA 2026 국제인공지능대전(서울 COEX)** | 학교 대표 부스 프로젝트 설명 도우미
- **경북소프트웨어마이스터고 매점 프로젝트** (진행 중) | JA Korea 창업 프로젝트의 지원을 받아 진행
- **2026.06 경북 청소년 창업 해커톤 경진대회** | 창의상
- **2026.06 전공 동아리 주간 프레임 동아리 1등** | MAJOR 프로젝트 기획 · 득표율 90%
- **2026.08 GEEKS 해커톤** | CokZip 학습 도움 튜터 기획 및 개발 참여

---

## 📬 Contact

- 프로젝트 협업, 해커톤, 팀 프로젝트 제안은 **contact@dlfkd.dev**로 연락 주세요.

<div align="center">

<a href="mailto:contact@dlfkd.dev">
  <img src="https://img.shields.io/badge/CONTACT-contact%40dlfkd.dev-0EA5E9?style=for-the-badge&logo=gmail&logoColor=white" alt="contact@dlfkd.dev"/>
</a>

</div>

---

## 🧭 지금 집중하는 것 (Current Focus)

- 창업 프로젝트를 통해 팀 단위 개발과 협업 경험 쌓기
- React 기반 웹 UI와 백엔드 API, 데이터 흐름 이해하기
- 학교에서 실제로 사용할 수 있는 서비스를 완성하고 운영해보기

<div align="center">

**아직 배우는 중이지만, 배우면서 만들고, 만들면서 더 배웁니다.**

</div>
